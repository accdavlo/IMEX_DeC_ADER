...
We will use equispaced (eq) or Gauss-Lobatto (GLB) nodes as quadrature nodes. \todo{We can delete the appendix A1 and they only that the intersect as done in our work the sdec is not really as interesting maybe sader...}
...
Therefore, we can conclude, that the IMEX ADER methods fulfill these requirements to a certain \todo{``These requirements'' is very vague, write more explicitly what you refer to.} extent, while the DeC and sDeC methods have their limitations with $\mathcal{D}_0= \emptyset$ in every case and bounded stability regions for most of the cases in the scope of $\mathcal{D}_1$. Nevertheless, we need to keep in mind that the set conditions are very strict, so,  the methods might still be applicable to some stiff equations. % \\
