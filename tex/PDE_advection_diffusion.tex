...
\end{equation}
where $a$ is the coefficient of the advection term and $d$ the coefficient of the diffusion term, using the von  Neumann stability analysis. %principles.
...
\begin{definition}[Scheme notation]	\label{defi: notation_pde_method}
	...
	\end{itemize}
\end{definition}
\begin{example}[Stability of IMEX DeC3 with $A_1$ and $D_2$ operators]
	\begin{figure}
		\centering
...
We can conclude that increasing the order of the time-marching method results in higher values for both $C_0$ and $E_0$. However, the considered higher order finite differences for the spatial discretizations do not grant significant improvements. 
